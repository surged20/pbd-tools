...
interface StatblockData {
    ...
    recalls: { skill: string; dc: number }[];
...
function extractStatblockData(actor: NPCPF2e): StatblockData {
    ...
    const traits = system.traits.value
        .map((trait) =>
            typeof trait === "string"
                ? trait
                : (trait as { name?: string; label?: string }).name ||
                  (trait as { name?: string; label?: string }).label ||
                  String(trait),
        )
        .filter(
            (trait) =>
                trait && !legacyAlignmentTraits.includes(trait.toLowerCase()),
        );
    ...
    // Perception
    const perception =
        (system.attributes as { perception?: { value?: number } }).perception
            ?.value || system.abilities.wis.mod;

    // Senses
    const senses = Object.values(
        (system.traits as { senses?: Record<string, unknown> }).senses || {},
    )
        .map((sense) =>
            typeof sense === "string"
                ? sense
                : (sense as { label?: string; type?: string }).label ||
                  (sense as { label?: string; type?: string }).type,
        )
        .filter(Boolean) as string[];

    // Languages
    const languages =
        (system.traits as { languages?: { value?: string[] } }).languages
            ?.value || [];
...
function extractRecallKnowledge(
    actor: NPCPF2e,
): { skill: string; dc: number }[] {
    const recalls: { skill: string; dc: number }[] = [];
    const system = actor.system;
    const level = system.details.level.value;
    ...
    const identification = (
        system as { details?: { identification?: Record<string, unknown> } }
    ).details?.identification;
    if (identification) {
        // Extract skills and their DCs from identification data
        Object.entries(
            (identification as { skills?: Record<string, unknown> }).skills ||
                {},
        ).forEach(([skill, data]) => {
            if (data && typeof data === "object" && "dc" in data) {
                recalls.push({
                    skill: skill.charAt(0).toUpperCase() + skill.slice(1),
                    dc: (data as { dc?: number }).dc || standardDC,
                });
            }
        });
    }
    ...
    if (recalls.length === 0) {
        const creatureType =
            (
                system.details as { creatureType?: string }
            ).creatureType?.toLowerCase() || "humanoid";
...
function extractAttacks(actor: NPCPF2e): {
    name: string;
    type: string;
    bonus: number;
    traits: string[];
    ...
    actor.itemTypes.melee?.forEach((strike) => {
        ...
        const isRanged = traits.some((trait) =>
            typeof trait === "string"
                ? ["ranged", "thrown", "propulsive"].includes(
                      trait.toLowerCase(),
                  )
                : ["ranged", "thrown", "propulsive"].includes(
                      (
        ...
        if (damageRolls && Object.keys(damageRolls).length > 0) {
            const damageStrings = Object.values(damageRolls).map((roll) => {
                ...
                if ((roll as unknown as { critical?: string }).critical)
                    str += ` plus ${(roll as unknown as { critical: string }).critical}`;
                return str;
            });
...
function extractActions(actor: NPCPF2e): {
    name: string;
    actionType: string;
    traits: string[];
    description: string;
...
function extractSpells(actor: NPCPF2e): {
    level: number;
    spells: string[];
    dc?: number;
    attack?: number;
...
function buildStatblockMessage(data: StatblockData): string {
    ...
    lines.push("");
    lines.push(
        `**AC** ${data.ac} **HP** ${data.hp.value}/${data.hp.max}${data.hp.details ? ` (${data.hp.details})` : ""}`,
    );

    // Saves
    lines.push(
        `**Saves** Fort ${data.saves.fortitude >= 0 ? "+" : ""}${data.saves.fortitude}, Ref ${data.saves.reflex >= 0 ? "+" : ""}${data.saves.reflex}, Will ${data.saves.will >= 0 ? "+" : ""}${data.saves.will}`,
    );
    ...
    if (Object.keys(data.speeds).length > 0) {
        const speedEntries = Object.entries(data.speeds)
            .map(([type, speed]) =>
                type === "land" ? `${speed} ft` : `${type} ${speed} ft`,
            )
            .join(", ");
    ...
    if (data.recalls.length > 0) {
        const recallEntries = data.recalls.map(
            (recall) => `${recall.skill} DC ${recall.dc}`,
        );
    ...
    if (data.spells && data.spells.length > 0) {
        ...
        data.spells.forEach((level) => {
            const levelName =
                level.level === 0
                    ? "Cantrips"
                    : `${level.level}${getOrdinalSuffix(level.level)} Level`;
