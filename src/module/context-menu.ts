...
class PBDContextMenu {
    ...
    build(options) {
        (this.menu = document.createElement("menu")), this.menu.classList.add("context-menu"), options.forEach((option) => this.buildOption(option)), document.body.appendChild(this.menu);
    }
    buildOption(option) {
        const li = document.createElement("LI");
        li.classList.add("context-menu-item"), li.addEventListener("click", async () => {
        ...
        i.classList.add("context-menu-icon"), i.classList.add("fa-brands"), i.classList.add("fa-discord");
        const span = document.createElement("span");
        span.classList.add("context-menu-text"), (span.textContent = option.label), button.appendChild(i), button.appendChild(span), li.appendChild(button), this.menu.appendChild(li);
    }
    show(x, y) {
        ...
        x + mw > w && (x -= mw), y + mh > h && (y -= mh), (this.menu.style.left = x + "px"), (this.menu.style.top = y + "px"), this.menu.classList.add("show-context-menu");
    }
    hide() {
