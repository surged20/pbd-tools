...
const config = Vite.defineConfig(({ command, mode }): Vite.UserConfig => {
                    ...
                    if (context.file.endsWith("en.json")) {
                        ...
                        fs.promises.copyFile(context.file, `${outDir}/${basePath}`).then(() => {
                            context.server.ws.send({
                                type: "custom",
                                event: "lang-update",
                                data: { path: `modules/pbd-tools/${basePath}` },
                            });
                        });
    ...
    if (command === "serve") {
        ...
        fs.writeFileSync("./styles/pbd-tools.css", `/** ${message} */\n`);
        fs.writeFileSync("./pbd-tools.mjs", `/** ${message} */\n\nimport "./src/pbd-tools.ts";\n`);
        fs.writeFileSync("./vendor.mjs", `/** ${message} */\n`);
    }

    return {
        base: command === "build" ? "./" : "/modules/pbd-tools/",
        publicDir: "static",
        define: {
        ...
        server: {
            ...
            open: "/game",
            proxy: {
                "^(?!/modules/pbd-tools/)": "http://localhost:30000/",
                "/socket.io": {
